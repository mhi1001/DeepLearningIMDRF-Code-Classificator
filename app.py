...
def run_model_script(script_name):
    ...
    result = subprocess.run(
        ["python", script_name], capture_output=True, text=True, encoding="utf-8"
    )
...
def classify():
    ...
    top_5_codes = (
        top_5_codes.tolist() if hasattr(top_5_codes, "tolist") else list(top_5_codes)
    )
    top_5_probs = (
        top_5_probs.tolist() if hasattr(top_5_probs, "tolist") else list(top_5_probs)
    )
